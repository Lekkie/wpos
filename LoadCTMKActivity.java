 ...
 */
public class LoadCTMKActivity extends BaseActivity {
    ...
    protected void initView() {
        ...
        titleNameText.setText("Load CTMK");
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_num_e:
            case R.id.btn_num_f:
                if(ctmk != null && ctmk.length() < 32){
                    ctmk = ctmk + ((Button) v).getText().toString();
                    inputMoneySetText();
                }
                break;
            ...
            case R.id.btn_num_delete:
                if(!ctmk.isEmpty()){
                    ctmk = ctmk.substring(0, ctmk.length() - 1);
                    inputMoneySetText();
                }
                break;
    ...
    {
        if(ctmk.length() < 17)
            key2Text.setText(ctmk);
        else
            key1Text.setText(ctmk.substring(16));
    }


    private int loadCTMK(){

        //ctmk = "DBEECACCB4210977ACE73A1D873CA59F"; //NIBSS Test CTMK
        if(ctmk != null && ctmk.length() == 32){
            try {
                int ret = KeyUtils.saveCTMK(mKey, ctmk, null, null);
                if(ret == 0){
                    showToast("CTMK load Success!");
                    GlobalData.getInstance().setLocalMasterKeyLoadedFlag(true);
                    return 0;
                }else {
